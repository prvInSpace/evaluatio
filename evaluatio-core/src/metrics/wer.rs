...
pub fn word_error_rate_per_pair(predictions: &Vec<&str>, references: &Vec<&str>) -> Vec<f64> {
...
pub fn word_edit_distance_per_pair(predictions: &Vec<&str>, references: &Vec<&str>) -> Vec<usize> {
    let predictions_split = split_strings_into_word_vec(predictions);
    let references_split = split_strings_into_word_vec(references);
