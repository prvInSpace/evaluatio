...
| Metrics | Details |
| --- | --- |
[WER](/metrics/wer.md) | Word error rate (WER) is the most commonly used metric in ASR. It quantifies the number word/token level errors divided by the length of the reference string.
...
Since [WER](/metrics/wer.md) is the most commonly used metric, this guide will use that as an example.

## Corpus-level vs utterance level evaluation

Certain metrics (e.g. [WER](/metrics/wer.md) and [CER](/metrics/cer.md)) calculates the summary corpus level statistic differently than a simple mean of the utterance level results. As such, it is important to include these corpus level results. Confidence intervals are normally derived from bootstrap corpus-level results.

However, when comparing multiple models we need to estimate the uncertainty of the results which means we need to use utterance level results in these cases.
...
Most metrics used for ASR have distributional properties that make various statistical inference tests unreliable for ASR purposes. Metrics such as [WER](/metrics/wer.md) have their own section about this on their page, so please refer to the page for the metric that you are using. In general, tests such as $t$-tests, Wilcoxon signed-rank test, etc. are not reliable for edit distance based metrics such as [WER](/metrics/wer.md) and [CER](/metrics/cer.md). Paired bootstrap test has been shown to be generally reliable and should be used over most other alternatives.
...
When running multiple comparisons, be that testing multiple models (3 models or more), comparing multiple dialects, etc., you should always correct for multiple testing. This is because when you are testing multiple hypotheses, the familywise error rate (FWER) becomes higher than the alpha leading to unreliable results. E.g. if you are testing 6 different dialects, the FMWR is actually $1−(1−0.05)^6=34\%$ rather than the desired alpha of 5%.

For most ASR applications, Holm-Bonferroni is preferred over regular Bonferroni correction.
