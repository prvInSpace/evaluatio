...
def test_holm_pvalue_out_of_bounds():
    ...
    with pytest.raises(ValueError):
        multiple_testing.holm_correction([-0.1, 0.4], 0.05)
    with pytest.raises(ValueError):
        multiple_testing.holm_correction([1.1, 0.4], 0.05)
...
def test_bonferroni_pvalue_out_of_bounds():
    ...
    with pytest.raises(ValueError):
        multiple_testing.bonferroni_correction([-0.1, 0.4], 0.05)
    with pytest.raises(ValueError):
        multiple_testing.bonferroni_correction([1.1, 0.4], 0.05)
