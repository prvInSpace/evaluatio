import pytest
from evaluatio.metrics.bleu import bootstrap_bleu
...
def test_mismatched_lengths_hyp1_raises():
    ...
    with pytest.raises(ValueError, match="same length"):
        bootstrap_bleu(references, hyp1, hyp2, iterations=999)
...
def test_generator_references_accepted(clearly_different_hypotheses):
    ...
    result = bootstrap_bleu((r for r in references), hyp1, hyp2, iterations=999)
    assert isinstance(result, float)


def test_generator_hypotheses_accepted(clearly_different_hypotheses):
    references, hyp1, hyp2 = clearly_different_hypotheses
    result = bootstrap_bleu(
        references,
        (h for h in hyp1),
        (h for h in hyp2),
        iterations=999,
...
def test_multiple_references_per_sentence_accepted():
    ...
    result = bootstrap_bleu(references, hyp1, hyp2, iterations=999)
    assert isinstance(result, float)
...
def test_identical_systems_not_significant(identical_references_and_hypotheses):
    ...
    result = bootstrap_bleu(references, hyp, hyp, iterations=4999)
...
def test_clearly_different_systems_significant(clearly_different_hypotheses):
    references, hyp1, hyp2 = clearly_different_hypotheses
    result = bootstrap_bleu(references, hyp1, hyp2, iterations=4999)
    assert (
        result < 0.05
...
def test_symmetry(clearly_different_hypotheses):
    ...
    references, hyp1, hyp2 = clearly_different_hypotheses
    p_12 = bootstrap_bleu(references, hyp1, hyp2, iterations=9999)
    p_21 = bootstrap_bleu(references, hyp2, hyp1, iterations=9999)
